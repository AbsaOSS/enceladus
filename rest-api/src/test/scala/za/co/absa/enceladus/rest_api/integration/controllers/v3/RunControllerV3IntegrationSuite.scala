...
class RunControllerV3IntegrationSuite extends BaseRestApiTestV3 with Matchers {
  ...
  // todo add sparkId filter cases
  // todo add uniqueId filter cases
  ...
  // todo add startDate filter cases
  ...
  s"GET $apiUrl/{datasetName}/{datasetVersion}/{runId}" can {
    ...
    "return 404" when {
      ...
      "the runId is not a valid numeric type" in {
        ...
        val response = sendGet[String](s"$apiUrl/dataset/1/runIdAsdf")

        assertBadRequest(response)
        response.getBody should include("Cannot convert 'runIdAsdf' to a valid runId expression")
      }
    }
  }
    ...
    "return 200" when {
      "there are Runs with the specified datasetName and datasetVersion" should {
        "return the Run with the latest RunId" in {
          ...
          val response = sendGet[String](s"$apiUrl/dataset/1/latest")
  ...
  s"POST $apiUrl/{datasetName}/{datasetVersion}" can {
    "return 201" when {
      "a new Run is created" should {
        "return the created Run" in {
          val run = RunFactory.getDummyRun(dataset = "datasetA", datasetVersion = 2)

          val response = sendPost[Run, Run](s"$apiUrl/datasetA/2", bodyOpt = Option(run))
        ...
        "provide a uniqueId if none is specified" in {
          val run = RunFactory.getDummyRun(dataset = "datasetA", datasetVersion = 2, uniqueId = None)
        ...
        "override any specified runId in favor of a generated one" which {
          ...
          "handles two runs being started simultaneously" in {
            val run1 = RunFactory.getDummyRun(dataset = "datasetA", datasetVersion = 2)
            val run2 = RunFactory.getDummyRun(dataset = "datasetA", datasetVersion = 2)

            val eventualResponse1 = sendPostAsync[Run, Run](s"$apiUrl/datasetA/2", bodyOpt = Option(run1))
            val eventualResponse2 = sendPostAsync[Run, Run](s"$apiUrl/datasetA/2", bodyOpt = Option(run2))
    ...
    "return 400" when {
      "receiving an empty JSON" in {
        val response = sendPost[String, String](s"$apiUrl/datasetA/2", bodyOpt = Option("{}"))

        assertBadRequest(response)

        val body = response.getBody
        body should include("mismatch: 'datasetA' != 'null'")
      }
