...
class UDFLibrary()(implicit val spark: SparkSession) {
  UDFLibrary.registerUdf() // re-registration would be unnecessary
}
...
object UDFLibrary {
  ...
  private var registry: Set[Int] = Set.empty

  private def shouldRegister(forSpark: SparkSession): Boolean = {
    var result = false
    this.synchronized {
      val hash = forSpark.hashCode() //using hash as sufficiently unique and allowing garbage collection
  ...
  private val errorColumnAppendUdfFn = new UDF2[Seq[Row], Row, Seq[Row]] {
