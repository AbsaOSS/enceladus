...
import za.co.absa.enceladus.utils.error.UDFunctionNames
...
case class NegationRuleInterpreter(rule: NegationConformanceRule) extends RuleInterpreter {
  ...
  override def conform(df: Dataset[Row])
    ...
    val negationErrUdfCall = callUDF(UDFunctionNames.confNegErr, lit(rule.outputColumn), col(rule.inputColumn))
    val errCol = "errCol"
