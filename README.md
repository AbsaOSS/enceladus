...
| --conf-spark-memory-fraction **value** | **Adnavced**. Fraction of (heap space - 300MB) used for execution and storage (default=`0.6`). Sets `spark.memory.fraction` Spark configuration parameter. [See the detailed description here](spark.memory.fraction). |
