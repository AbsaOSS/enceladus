...
sap.ui.controller("components.mappingTable.mappingTableMain", {
	...
	sap.ui.getCore().byId("newMappingTableHdfsSelector").attachToggleOpenState(this.hdfsPathOpen, this)
	sap.ui.getCore().byId("newMappingTableHdfsSelector").attachSelectionChange(this.hdfsPathSelect, this)
	...
	sap.ui.getCore().byId("newDefaultValueFieldSelector").attachSelectionChange(this.targetColSelect, this)
	...
	var helper = function(aToks, sModelPathAcc) {
		...
		for ( var i in items) {
		    ...
		    var modelPath = sModelPathAcc.substring(0, sModelPathAcc.length - 9) // substring to get rid of the children suffix
		    if (p === modelPath) {
	...
	this._model.setProperty("/newMappingTable/hdfsPath", sHdfsPath)
    },
	...
	// This is a workaround for a bug in the Tree component of 1.56.5
	// TODO: verify whether this was fixed in the subsequent versions
	var tree = sap.ui.getCore().byId("newMappingTableHdfsSelector")
	var items = tree.getItems()
	for ( var i in items) {
	    items[i].setSelected(false)
	}
	...
	var items = sap.ui.getCore().byId("newMappingTableHdfsSelector").getItems();
	for ( var ind in items) {
	    items[ind].setHighlight(sap.ui.core.MessageType.None)
	}
    },
	...
	sap.ui.getCore().byId("newMappingTableHdfsSelector").collapseAll();

	GenericService.treeNavigateTo(current.hdfsPath, "/newMappingTable/HDFS", "newMappingTableHdfsSelector", this)
    },
	...
	sap.ui.getCore().byId("newMappingTableHdfsSelector").expandToLevel(1);
