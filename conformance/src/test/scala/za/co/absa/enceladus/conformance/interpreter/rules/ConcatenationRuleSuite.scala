...
class ConcatenationRuleSuite extends FunSuite with SparkTestBase with TestRuleBehaviors {
  ...
  val concatRule = ConcatenationConformanceRule(order = 1, outputColumn = "CombinedName", controlCheckpoint = false, Seq("name", "city", "address"))
  val concatArrayRule = ConcatenationConformanceRule(order = 2, outputColumn = "rooms.CombinedLabel", controlCheckpoint = false, Seq("rooms.roomName", "rooms.label"))
  val concatDeepArrayRule = ConcatenationConformanceRule(order = 3, outputColumn = "rooms.books.CombinedBooks", controlCheckpoint = false, Seq("rooms.books.author", "rooms.books.name"))

  val concatRulesList = List(concatRule, concatArrayRule, concatDeepArrayRule)

  val concatOrdersDS = Dataset(name = "Library Conformance", version = 1, hdfsPath = "src/test/testData/library", hdfsPublishPath =
    "testData/conformedLibrary",
    schemaName = "Library", schemaVersion = 1,
    conformance = concatRulesList)

  val conformedConcatOrdersJSON: String =
  ...
  val inputDf: DataFrame = spark.createDataFrame(DeepArraySamples.libraryData)
