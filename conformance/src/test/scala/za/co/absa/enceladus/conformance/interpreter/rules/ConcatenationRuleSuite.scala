...
class ConcatenationRuleSuite extends FunSuite with SparkTestBase with TestRuleBehaviors {
  ...
  private val concatRule = ConcatenationConformanceRule(order = 1, outputColumn = "CombinedName", controlCheckpoint = false, Seq("name", "city", "address"))
  private val concatArrayRule = ConcatenationConformanceRule(order = 2, outputColumn = "rooms.CombinedLabel", controlCheckpoint = false, Seq("rooms.roomName", "rooms.label"))
  private val concatDeepArrayRule = ConcatenationConformanceRule(order = 3, outputColumn = "rooms.books.CombinedBooks", controlCheckpoint = false, Seq("rooms.books.author", "rooms.books.name"))

  private val concatRulesList = List(concatRule, concatArrayRule, concatDeepArrayRule)

  private val concatOrdersDS = Dataset(name = "Library Conformance", version = 1, hdfsPath = "src/test/testData/library", hdfsPublishPath =
