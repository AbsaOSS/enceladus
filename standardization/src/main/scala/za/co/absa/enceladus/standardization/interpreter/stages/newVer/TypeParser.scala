...
package za.co.absa.enceladus.standardization.interpreter.stages.newVer
...
sealed trait TypeParser {
  ...
  val path: String
  val parent: TypeParser.Parent
  val fieldName: String = SchemaUtils.getFieldNameOverriddenByMetadata(field)
  val currentAttrPath: String = appendPath(path, fieldName) // absolute path to field to parse
  val column: Column = parent.map {
    case Left(x) => x //parent is array
    case Right(x) => x(fieldName) //parent is struct
  }.getOrElse(col(currentAttrPath)) // no parent
  val isArrayElement: Boolean = parent.exists(_.isLeft)
}
...
object TypeParser {
  ...
  private def apply(field: StructField, path: String, origSchema: StructType, parent: Parent = None): TypeParser = {
  ...
  private final case class ArrayParser(field: StructField,
                                       ...
                                       parent: Parent) extends TypeParser {
    ...
    override def standardize(): ParseOutput = {
      ...
      val lambda = (forCol: Column) => TypeParser(arrayField, path, origSchema, Parent.array(forCol)).standardize()
  ...
  private final case class StructParser(field: StructField,
    ...
    override def standardize(): ParseOutput = {
      val out =  fieldType.fields.map(TypeParser(_, currentAttrPath, origSchema, Parent.struct(column)).standardize())
  ...
  private final case class NumericParser(field: StructField,
                                         path: String,
                                         origSchema: StructType,
                                         parent: Parent) extends SimpleParser
  ...
  private trait DateTimeParser extends PrimitiveParser {
    ...
    //protected val defaultTimeZone: Option[String] = pattern.defaultTimeZone
  ...
  private final case class DateParser(field: StructField,
                                      path: String,
                                      origSchema: StructType,
                                      parent: Parent) extends DateTimeParser {
